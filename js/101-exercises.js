...
function isMultipleOfBothThreeAndFive(input){
...
const startsWithVowel = input => /[aeiou]/i.test(input[0]);
...
const endsWithVowel = input => /[aeiou]/i.test(input[input.length - 1]);
...
const startsAndEndsWithVowel = input => startsWithVowel(input) && endsWithVowel(input);
...
const secondToLast = input => input[input.length - 2];
...
const thirdToLast = input => input[input.length - 3];
...
const firstAndSecond = array => [array[0], array[1]];
...
function getUniqueValues(array){
    let uniquelyYou = [];
    for (let string of array) {
        if (uniquelyYou.includes(string)){
            continue;
        }
        uniquelyYou.push(string);
    }
    return uniquelyYou;
}
...
function getYearPublished(paper){
...
function highestPriceBook(books){
    let pricyBook = {price:0}
    for (let book of books) {
        if (book.price > pricyBook.price) {
            pricyBook = book;
        }
    }return pricyBook;
}
...
function lowestPriceBook(books){
    let cheapBook = {price: Infinity};
    for (let book of books){
        if (book.price < cheapBook.price){
            cheapBook = book;
        }
    }
    return cheapBook;
...
function getAverageItemPrice(cart){
    let total = 0
    let itemTypes = cart.items.length;
    for (let item of cart.items){
        total += item.price;
    }
    return total / itemTypes;
}
...
function mostSpentOnItem(cart){
    let spensivItem = {price:0,
                    quantity:0};
    for (let item of cart.items){
        if((item.price * item.quantity)>(spensivItem.price * spensivItem.quantity)){
            spensivItem = item;
        }
    } return spensivItem;
}
